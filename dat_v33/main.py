...
# All tunable weights live in params.py (same values as v20).
from params import *
...
def observe():
    ...
    for t in ct.get_tiles():
        ...
        if part is not None and part.get_id() != me:
            ...
            if part.is_head():
                ...
                if enemy and seg_count.get(pid, 0) > ct.get_length() + THREAT_LEN_MARGIN:
    ...
    if visible_pearls >= MIN_PEARL_CLUSTER and rng.random() < SONAR_PING_PROB:
...
def choose():
    ...
    for d in range(4):
        ...
        if j == -2:
            ...
            if rnd < PORTAL_EXPLORE_ROUNDS:
                s = (EXPLORE_W * PORTAL_EXPLORE_MULT) + rng.random()
            else:
                # Gradual Paranoia: Ramp up the penalty steadily from round 50 to 250
                curiosity_factor = min((rnd - PORTAL_EXPLORE_ROUNDS) / PORTAL_PARANOIA_RAMP, 1.0)
            ...
            if j != nb(head, d):
                ...
                if staleness > PORTAL_STALE_GRACE:
            ...
            if DEADEND_PEN and len(nbrs(j)) <= 2 and dead_end(j, head):
                s -= DEADEND_PEN
            ...
            if r < length + LETHAL_SLACK and j != nb(head, d):
                s += (PORTAL_PEN + ENDGAME_PORTAL_PEN) * pmult
...
def maybe_split():
    global splits_done
    rnd = game.get_round_num()
    if is_long(ct.get_length(), rnd):
        return False
    if is_anchor():
        if splits_done >= ANCHOR_SPLITS or rnd > ANCHOR_SPLIT_UNTIL or rnd >= ENDGAME_ROUND:
            return False
        if ct.get_length() < ANCHOR_SPLIT_AT or ct.get_unit_count() >= unit_cap():
            return False
        if not ct.can_split(ANCHOR_CHILD):
            return False
    ...
    if rnd > SPLIT_UNTIL or rnd >= ENDGAME_ROUND:
        return False
    if ct.get_length() < SPLIT_AT or ct.get_unit_count() >= unit_cap():
        return False
