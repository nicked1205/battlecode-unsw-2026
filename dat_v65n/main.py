...
seen_round = []
ptime = []
cmax = []            # max countdown ever observed per tile (-1 unknown): small => hot spawn tile
pearls = {}
others = {}          
heads = {}
seg_count = {}
...
def setup():
    global WID, HEI, hedge, vedge, seen_round, ptime, LAST_ROW, cmax, MAX_UNITS, MIN_SWARM_UNITS
    WID, HEI = game.get_map_size()
    global CASH_ROUND
    if WID * HEI >= CASH_BIG_AREA:
        CASH_ROUND = CASH_ROUND_BIG
    n = WID * HEI
    ...
    SD_MASK[:] = [0] * n
    if PRIOR_ENABLE:
        mapprior.init(WID, HEI, ct.get_id())
...
BITS = [[d for d in range(4) if m >> d & 1] for m in range(16)]
SD_STAMP = []
SD_DIST = []
SD_MASK = []
sd_cur = 0
...
def search(head, blocked, vacate, targets=()):
    if not SYM_SEARCH:
        return search_old(head, blocked, vacate, targets)
    # BFS that credits every tile to ALL first moves lying on a shortest path to it,
    # so no direction (e.g. West, last in the order) is starved by tie-breaking.
    global sd_cur
    sd_cur += 1
    cur = sd_cur
    stamp = SD_STAMP
    ...
    mask = SD_MASK
    rnd = game.get_round_num()
    node_cap = SEARCH_NODES // 2 if (FIRST_LIGHT and len(traj) <= 1) else SEARCH_NODES
    best = [0.0] * 4
    unknown = [0] * 4
    tdist = [1 << 20] * 4
    stamp[head] = cur
    ...
    for d in range(4):
        j = step(head, d)
        if j < 0 or not passable(j, 1, blocked, vacate):
            continue
        if stamp[j] == cur:
            if dist_of[j] == 1:
                mask[j] |= 1 << d
    ...
    while q and expanded < node_cap:
        idx = pop()
        dist = dist_of[idx]
        m = mask[idx]
        bits = BITS[m]
        expanded += 1
        if targets and idx in targets:
            for f in bits:
                if dist < tdist[f]:
                    tdist[f] = dist
        if curiosity_pull > 0:
        ...
        if sr < 0:
            ...
            if pt >= 0:
                ...
                if 0 <= pred <= SPAWN_HORIZON:
                    v2 = SPAWN_W / (1 + (pred if pred > dist else dist))
                    if v2 > v:
                        v = v2
                elif pred < 0 and EXPIRED_ENABLE and pr is None:
                    v2 = EXPIRED_W / ((1 + dist) * (1 + (-pred) / EXPIRED_TAU))
                    if v2 > v:
                        v = v2
            if v > 0.0:
                for f in bits:
...
def search_old(head, blocked, vacate, targets=()):
    rnd = game.get_round_num()
    node_cap = SEARCH_NODES // 2 if (FIRST_LIGHT and len(traj) <= 1) else SEARCH_NODES
    best = [0.0] * 4
    unknown = [0] * 4
        ...
        if sr < 0:
            ...
            if pt >= 0:
                ...
                if 0 <= pred <= SPAWN_HORIZON:
                    ...
                    # Countdown ran out while unobserved: a pearl probably spawned there
                    v = EXPIRED_W / ((1 + dist) * (1 + (-pred) / EXPIRED_TAU))
                    if v > best[first]:
                        best[first] = v
        nd = dist + 1
        for j in nbrs(idx):
...
def is_long(length, rnd):
    if not LONG_ENABLE or rnd < LONG_ROUND:
        return False
    return (LONG_ANCHOR_AUTO and is_anchor()) or length >= LONG_MIN_LEN
...
# ==========================================
# MAP PRIOR: walk idle dragons straight to precomputed rich regions (known ladder maps only)
# ==========================================
pull_field = None    # distance field (bytes) of this dragon's target region, or None = no pull this turn
pull_hd = 0          # its value at our head

def prior_update():
    """Sets pull_field for this turn. Any unknown map / mismatch leaves it None (= v65x behaviour)."""
    global pull_field, pull_hd
    pull_field = None
    mp = mapprior
    if mp.dead:
        return
    rnd = game.get_round_num()
    if not mp.verify(ct.get_tiles(), rnd) or rnd >= PRIOR_UNTIL:
        return
    length = ct.get_length()
    if length > PRIOR_MAXLEN or is_long(length, rnd) or is_king(length, rnd):
        return
    head = traj[-1]
    if mp.target < 0:
        if mp.arrived:
            return
        if PRIOR_FRAC < 1.0 and mp.prng.random() >= PRIOR_FRAC:
            # not a traveller: this dragon stays local (children decide for themselves)
            mp.arrived = True
            return
        mp.target = mp.pick(head, rnd, (), PRIOR_MAXD)
        if mp.target < 0:
            return
    t = mp.target
    F = mp.field(t)
    dh = F[head]
    if not mp.arrived and dh <= PRIOR_CAP_D:
        # cap: enough of our heads already sit in this region -> pick another one
        lim = PRIOR_ARRIVE + 1
        n = 0
        for h, (en, _, _) in heads.items():
            if not en and F[h] <= lim:
                n += 1
        if n >= mp.tgts[t][3]:
            mp.full_until[t] = rnd + PRIOR_FULL_TTL
            mp.target = t = mp.pick(head, rnd, (t,), PRIOR_MAXD)
            if t < 0:
                mp.arrived = True
                return
            F = mp.field(t)
    ...
    if dh <= PRIOR_ARRIVE:
        mp.arrived = True
    if mp.arrived and (PRIOR_LEASH <= 0 or dh <= PRIOR_LEASH):
        return
    for h, (en, _, _) in heads.items():
        if en:
            if wdist(head, h) <= PRIOR_DANGER:
                return
        elif PRIOR_MATE_R > 0 and wdist(head, h) <= PRIOR_MATE_R:
            return   # a friendly head close by: let the normal spacing logic act (corridor head-ons)
    pull_field = F
    pull_hd = dh
...
def prior_bonus(j):
    ...
    if dj >= 255:
        return 0.0
    g = pull_hd - dj
    return PRIOR_W * (1 if g > 0 else (-1 if g < 0 else 0))
...
def choose():
    ...
    anchor = is_anchor() and length >= ANCHOR_CAUTION_MINLEN
    ...
    pull = pull_field is not None and (PRIOR_IDLE_V <= 0 or max(pearl_val) < PRIOR_IDLE_V)
        ...
        if j == -2:
            ...
            for enemy, eid, _ in head_threats(j):
                if enemy and eid in prey_ids:
                    s += HUNT_ADJ_W
                elif enemy and HR_EVEN_MULT != 1.0 and seg_count.get(eid, 0) >= length:
                    # a head-on with an equal/bigger enemy is at worst an even trade
                    s -= head_risk * HR_EVEN_MULT
                else:
                    s -= head_risk
            ...
            if not (HOTVISIT_ENABLE and hot_near(j)):
